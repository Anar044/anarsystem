(function(){
 ...
 function removeLegacyStyles(){document.querySelectorAll('link[rel="stylesheet"]').forEach(link=>{const href=(link.getAttribute('href')||'').toLowerCase();if(href.includes('app-shell.css'))return;if(href.includes('pnl.css'))return;if(href.includes('qr-menu.css'))return;if(href.includes('plugin-control.css'))return;if(href.includes('supplier-balances.css'))return;if(href.includes('abc-xyz.css'))return;if(href.includes('cash.css'))return;if(href.includes('cash-history.css'))return;if(href.includes('cash-shifts.css'))return;if(href.includes('nakladnye.css'))return;if(href.includes('rashodnye-nakladnye.css'))return;if(isAuthPage()&&href.includes('auth.css'))return;link.remove();});const path=location.pathname.toLowerCase();if(!path.endsWith('/index.html')&&!path.endsWith('/')&&!isAuthPage())document.querySelectorAll('head > style').forEach(style=>{if(style.id!=='hc-unified-style'&&style.id!=='sh-mobile-responsive'&&style.id!=='cash-order-details-polish'&&style.id!=='incoming-invoices-critical-style')style.remove();});}
