...
const round=v=>Math.round((Number(v)||0)*100)/100,n=v=>Math.max(0,Number(v)||0);
...
async function api(body=null){const t=await token(),asOf=$('hcpAsOf').value||today(),o={headers:{Authorization:`Bearer ${t}`,Accept:'application/json'}};let url=`/api/hr/compensation?asOf=${encodeURIComponent(asOf)}`;if(body){o.method='POST';o.headers['Content-Type']='application/json';o.body=JSON.stringify({...body,asOf});url='/api/hr/compensation'}const r=await fetch(url,o),j=await r.json().catch(()=>({success:false,message:'Некорректный ответ API'}));if(!r.ok||!j.success)throw new Error(j.message||`HTTP ${r.status}`);return j}
function status(t,k=''){const e=$('hcpStatus');e.textContent=t;e.className=`hr-status ${k}`.trim()}
function error(t=''){const e=$('hcpError');e.hidden=!t;e.textContent=t}
...
function preview(){const c=formCalc(),o=c.official,d=c.combined;[['pvOfficialGross',o.gross],['pvIncomeTax',o.employee.incomeTax],['pvEmployeeSocial',o.employee.socialInsurance],['pvEmployeeUnemployment',o.employee.unemploymentInsurance],['pvEmployeeMedical',o.employee.medicalInsurance],['pvOfficialNet',o.net],['pvAdditionalGross',c.additional.gross],['pvAdditionalDeductions',c.additional.employeeDeductions],['pvAdditionalEmployer',c.additional.employerContributions],['pvAdditionalNet',c.additional.net],['pvEmployeeTotal',c.totalEmployeeReceives],['pvEmployerSocial',d.employer.socialInsurance],['pvEmployerUnemployment',d.employer.unemploymentInsurance],['pvEmployerMedical',d.employer.medicalInsurance],['pvEmployerCost',c.totalEmployerCost]].forEach(([id,v])=>$(id).textContent=money(v));const ex=$('hcpTaxTreatment').value==='EXEMPT_WITH_BASIS';$('hcpBasisWrap').hidden=!ex;$('hcpBasis').required=ex;$('pvTaxNote').textContent=ex?'Исключено из баз только по указанному законному основанию.':'Включено в налоговые и страховые базы.';$('hcpRuleBadge').textContent=`AZ · ${yearOf($('hcpAsOf').value||today())}`}
...
function selectors(){const ec=$('hcpEmployee').value,rc=$('hcpRole').value;$('hcpEmployee').innerHTML=(state.employees||[]).map(e=>`<option value="${esc(e.id)}">${esc(e.name)}${e.roleName?' · '+esc(e.roleName):''}</option>`).join('')||'<option value="">Нет сотрудников</option>';$('hcpRole').innerHTML=(state.roles||[]).map(r=>`<option value="${esc(r.code)}">${esc(r.name)} · ${Number(r.employeeCount||0)} сотр.</option>`).join('')||'<option value="">Нет должностей</option>';if((state.employees||[]).some(e=>e.id===ec))$('hcpEmployee').value=ec;if((state.roles||[]).some(r=>r.code===rc))$('hcpRole').value=rc}
...
function roleRows(){const l=state.roles||[];$('hcpRoleCount').textContent=`${l.length} должностей`;$('hcpRoleRows').innerHTML=l.map(r=>{if(!r.term||!r.calculation)return `<tr><td class="text-left"><div class="hr-name">${esc(r.name)}</div><div class="hr-sub">${esc(r.code)}</div></td><td>${r.employeeCount||0}</td><td colspan="5" class="text-left"><span class="hr-badge pending">Условия не настроены</span></td><td>—</td><td><button class="hr-link-button" data-rnew="${esc(r.code)}">Настроить</button></td></tr>`;const c=r.calculation,o=c.official;return `<tr><td class="text-left"><div class="hr-name">${esc(r.name)}</div><div class="hr-sub">${esc(r.code)}</div></td><td>${r.employeeCount||0}</td><td>${money(o.gross)}</td><td><b>${money(o.net)}</b></td><td>${money(r.term.additionalAmount)}</td><td><b>${money(c.totalEmployeeReceives)}</b></td><td><b>${money(c.totalEmployerCost)}</b></td><td>${taxBadge(r.term)}</td><td><div class="hcp-row-actions"><button class="hr-link-button" data-redit="${esc(r.code)}">Изменить</button><button class="hr-link-button danger" data-rdisable="${esc(r.code)}">Отключить</button></div></td></tr>`}).join('')||'<tr><td colspan="9" class="hr-empty">Должности не найдены.</td></tr>';$('hcpRoleRows').querySelectorAll('[data-rnew]').forEach(b=>b.onclick=()=>newRole(b.dataset.rnew));$('hcpRoleRows').querySelectorAll('[data-redit]').forEach(b=>b.onclick=()=>editRole(b.dataset.redit));$('hcpRoleRows').querySelectorAll('[data-rdisable]').forEach(b=>b.onclick=()=>disableRole(b.dataset.rdisable))}
function employeeRows(){const l=state.employees||[];$('hcpRowCount').textContent=`${l.length} сотрудников`;$('hcpRows').innerHTML=l.map(e=>{if(!e.term||!e.calculation){const a=e.roleCode?`<button class="hr-link-button" data-rnew="${esc(e.roleCode)}">Для должности</button>`:`<button class="hr-link-button" data-ind="${esc(e.id)}">Индивидуально</button>`;return `<tr><td class="text-left"><div class="hr-name">${esc(e.name)}</div><div class="hr-sub">${esc(e.roleName||'Без должности')} · ${esc(e.code||'')}</div></td><td>${sourceBadge(e)}</td><td colspan="10" class="text-left"><span class="hr-badge pending">Условия не настроены</span></td><td>${a}</td></tr>`}const c=e.calculation,o=c.official,a=e.sourceType==='EMPLOYEE'?`<div class="hcp-row-actions"><button class="hr-link-button" data-eedit="${esc(e.id)}">Изменить</button><button class="hr-link-button" data-inherit="${esc(e.id)}">Вернуть к должности</button></div>`:`<button class="hr-link-button" data-ind="${esc(e.id)}">Индивидуально</button>`;return `<tr><td class="text-left"><div class="hr-name">${esc(e.name)}</div><div class="hr-sub">${esc(e.roleName||'Без должности')} · ${esc(e.code||'')}</div></td><td>${sourceBadge(e)}</td><td>${money(o.gross)}</td><td>${money(o.employee.incomeTax)}</td><td>${money(o.employee.socialInsurance)}</td><td>${money(o.employee.unemploymentInsurance)}</td><td>${money(o.employee.medicalInsurance)}</td><td><b>${money(o.net)}</b></td><td>${money(e.term.additionalAmount)}</td><td><b>${money(c.totalEmployeeReceives)}</b></td><td>${money(c.combined.employer.total)}</td><td><b>${money(c.totalEmployerCost)}</b></td><td>${a}</td></tr>`}).join('')||'<tr><td colspan="13" class="hr-empty">Сотрудники не найдены.</td></tr>';$('hcpRows').querySelectorAll('[data-ind]').forEach(b=>b.onclick=()=>newEmployee(b.dataset.ind));$('hcpRows').querySelectorAll('[data-eedit]').forEach(b=>b.onclick=()=>editEmployee(b.dataset.eedit));$('hcpRows').querySelectorAll('[data-inherit]').forEach(b=>b.onclick=()=>inherit(b.dataset.inherit));$('hcpRows').querySelectorAll('[data-rnew]').forEach(b=>b.onclick=()=>newRole(b.dataset.rnew))}
...
function reset(s='ROLE'){const r=$('hcpRole').value,e=$('hcpEmployee').value;$('hcpId').value='';$('hcpScope').value=s;$('hcpFrom').value=$('hcpAsOf').value||today();$('hcpTo').value='';$('hcpOfficialGross').value=0;$('hcpAdditional').value=0;$('hcpMethod').value='CASH';$('hcpTaxTreatment').value='TAXABLE';$('hcpBasis').value='';$('hcpNote').value='';if(r)$('hcpRole').value=r;if(e)$('hcpEmployee').value=e;scope();preview()}
function fill(t){$('hcpId').value=t.id;$('hcpFrom').value=t.effectiveFrom;$('hcpTo').value=t.effectiveTo||'';$('hcpOfficialGross').value=t.officialGross;$('hcpAdditional').value=t.additionalAmount;$('hcpMethod').value=t.additionalPaymentMethod||'CASH';$('hcpTaxTreatment').value=t.additionalTaxTreatment||'TAXABLE';$('hcpBasis').value=t.additionalLegalBasis||'';$('hcpNote').value=t.note||'';preview();document.querySelector('.hcp-editor')?.scrollIntoView({behavior:'smooth',block:'start'})}
function newRole(code){reset('ROLE');if(code)$('hcpRole').value=code;document.querySelector('.hcp-editor')?.scrollIntoView({behavior:'smooth',block:'start'})}
function editRole(code){const r=(state.roles||[]).find(x=>x.code===code);if(!r?.term)return newRole(code);$('hcpScope').value='ROLE';scope();$('hcpRole').value=code;fill(r.term)}
