(() => {
  ...
  const scalar = value => {
    ...
    if (Array.isArray(value)) {
      for (const x of value) { const s = scalar(x); if (s) return s; }
      return null;
    }
    if (typeof value === 'object') {
      for (const k of ['name','Name','fullName','FullName','title','Title','value','Value']) {
        const s = scalar(value[k]);
        if (s) return s;
      }
    }
    return null;
  ...
  function details(e) {
    const d = e?.data ?? {};
    const parts = [];
    const item = deep(d, ['itemName','productName','dishName','menuItemName','name']);
    ...
    if (sum && !/created|neworder/i.test(String(e?.pluginEventType))) parts.push(`Сумма: ${sum}`);
  ...
  function render(events) {
    ...
    host.innerHTML = events.slice().sort((a,b) => {
      ...
      return `<div class="cm-event cm-event-rich">
        <div class="cm-event-main"><strong>${esc(title(e))}</strong><span class="cm-event-time">${esc(dt(when))}</span></div>
  ...
  const observer = new MutationObserver(() => {
    const host = document.getElementById('cm-history');
    if (!host) return;
    if (host.dataset.richHistory === '1') return;
    const text = host.textContent || '';
    if (!text || text.includes('История событий пока недоступна.')) return;
