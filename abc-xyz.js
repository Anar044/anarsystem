(function(){
  ...
  function extractRows(payload){
    ...
    const add=(candidate,columns)=>{
      ...
      if(typeof mapped[0]==='object'&&!Array.isArray(mapped[0])){for(const row of mapped){const key=JSON.stringify(row);if(!seen.has(key)){seen.add(key);out.push(row);}}}
    };
  ...
  function dishInfo(row){
    ...
    const group=String(pick(row,['DishGroup','Product.TopParent','Product.Hierarchy'])||'').trim()||'Без группы';
    const category=String(pick(row,['DishCategory','Product.Category','DishCategory.Accounting'])||'').trim()||'(без категории)';
  ...
  function buildFilters(){
    const filters=[{field:'DishType',operator:'IncludeValues',values:['DISH']},{field:'DeletedWithWriteoff',operator:'ExcludeValues',values:['DELETED_WITHOUT_WRITEOFF']}];
  ...
  async function query(){
    ...
    const body={
      ...
      groupByRowFields:['OpenDate.Typed','Store.Name','DishGroup','DishCategory','DishCode','DishName','DishMeasureUnit'],
      groupByColumnFields:[],
      aggregateFields:['DishSumInt','DishAmountInt','ProductCostBase.Profit'],
      filters:buildFilters()
    };
    const response=await fetch('/api/iiko/olap',{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify(body)});
    const text=await response.text();let data={};try{data=text?JSON.parse(text):{};}catch(_){throw new Error(text||`HTTP ${response.status}`);}
  ...
  function aggregate(rows){
    ...
    for(const row of rows){
      const info=dishInfo(row);const key=info.key;
      if(!map.has(key))map.set(key,{...info,revenue:0,quantity:0,profit:0,periods:new Map()});
      const x=map.get(key);x.revenue+=num(pick(row,['DishSumInt','Sales']));x.quantity+=num(pick(row,['DishAmountInt','DishAmountInt.PerOrder']));x.profit+=num(pick(row,['ProductCostBase.Profit','Profit']));
      const date=dateOf(row);const b=bucket(date,$('interval').value);if(b){if(!x.periods.has(b))x.periods.set(b,{revenue:0,quantity:0,profit:0});const p=x.periods.get(b);p.revenue+=num(pick(row,['DishSumInt','Sales']));p.quantity+=num(pick(row,['DishAmountInt','DishAmountInt.PerOrder']));p.profit+=num(pick(row,['ProductCostBase.Profit','Profit']));}
    }
    return [...map.values()];
  }

  function abcClass(value,total,a,b){
    if(!Number.isFinite(value)||value<=0||total<=0)return 'C';
    const cumulative=value;
    if(cumulative/total*100<=a)return 'A';
    if(cumulative/total*100<=b)return 'B';
    return 'C';
  }

  function assignABC(items,metric){
    const a=num($('abcA').value),b=num($('abcB').value);const total=items.reduce((s,x)=>s+Math.max(0,num(x[metric])),0);
    let cumulative=0;const sorted=[...items].sort((x,y)=>num(y[metric])-num(x[metric]));
    for(const item of sorted){const value=Math.max(0,num(item[metric]));cumulative+=value;item[`abc_${metric}`]=abcClass(cumulative,total,a,b);item[`share_${metric}`]=total?value/total*100:0;}
    return total;
  }
  ...
  function calculate(rows){
    const items=aggregate(rows);if(!items.length)return [];
  ...
  function updateFilterOptions(raw){const infos=raw.map(dishInfo);fillSelect('store',optionValues(infos,'store'));fillSelect('group',optionValues(infos,'group'));fillSelect('category',optionValues(infos,'category'));}

  function selectedABC(){return [...document.querySelectorAll('.indicator-box input:checked')].map(x=>x.value);}
  function visibleRows(){const q=($('search').value||'').trim().toLowerCase();if(!q)return resultRows;return resultRows.filter(x=>[x.name,x.code,x.group,x.category,x.store,x.xyz,x.matrix].join(' ').toLowerCase().includes(q));}
  ...
  function abcColumns(){return selectedABC();}
  function renderTable(){
    const head=$('#resultTable thead'),body=$('#resultTable tbody'),foot=$('#resultTable tfoot');const rows=visibleRows();
    let cols=`<th>Группа</th><th>Категория</th><th>Артикул</th><th class="text-left">Наименование</th><th>Ед. изм.</th>`;
    const metrics=abcColumns();
    ...
    body.innerHTML=rows.map(x=>{let html=`<td class="group-cell">${esc(x.group)}</td><td class="category-cell">${esc(x.category)}</td><td>${esc(x.code)}</td><td class="text-left"><b>${esc(x.name)}</b></td><td>${esc(x.unit)}</td>`;
    ...
    if(currentTab==='matrix')footHtml+='<th>—</th>';foot.innerHTML=`${footHtml}</tr>`;
  ...
  async function run(){
    const button=$('run');button.disabled=true;setStatus('Загрузка…','loading');$('error').hidden=true;
    try{const raw=await query();allRows=raw;updateFilterOptions(raw);resultRows=calculate(raw);render();setStatus(`Готово · ${resultRows.length} позиций`,'ok');}
    catch(error){resultRows=[];render();$('error').hidden=false;$('error').textContent=error?.message||String(error);setStatus('Ошибка','error');}
    finally{button.disabled=false;}
  }

  function exportCsv(){
    const rows=visibleRows();if(!rows.length)return;
    const metrics=abcColumns();const headers=['Группа','Категория','Артикул','Наименование','Ед. изм.'];
    ...
    if(currentTab==='xyz'||currentTab==='matrix')headers.push('Коэффициент вариации','XYZ');if(currentTab==='matrix')headers.push('Матрица');
    ...
    const csv='\uFEFF'+lines.map(r=>r.map(v=>`"${String(v??'').replace(/"/g,'""')}"`).join(';')).join('\n');const blob=new Blob([csv],{type:'text/csv;charset=utf-8'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=`abc-xyz-${$('from').value}-${$('to').value}.csv`;a.click();URL.revokeObjectURL(url);
  }
