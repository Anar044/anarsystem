(()=>{
  ...
  function renderDevices(){
    ...
    box.innerHTML=list.length?list.map(d=>`<article class="hr-device-card">
      <div><span>${esc(d.provider)}${d.model?' · '+esc(d.model):''}</span><strong>${esc(d.name)}</strong><small>${esc(d.location||'Место не указано')} · ${esc(d.timezone||'Asia/Baku')}</small>${d.serialNumber?`<small>SN: ${esc(d.serialNumber)} · ${esc(d.protocolMode||'PUSH_ADMS')} · порт ${Number(d.connectorPort||8088)}</small>`:'<small>Serial Number ещё не указан</small>'}</div>
  ...
  function configForDevice(deviceId,value){const d=(data.devices||[]).find(x=>x.id===deviceId);if(!d?.serialNumber)return'';return JSON.stringify({[d.serialNumber]:{enabled:true,name:d.name||'ZKTeco',model:d.model||'ZKTeco',deviceToken:String(value||'')}},null,2)}
  ...
  async function saveDevice(ev){ev.preventDefault();try{setStatus('Сохраняем…','loading');const result=await api({action:'saveDevice',provider:$('deviceProvider').value,name:$('deviceName').value,location:$('deviceLocation').value,timezone:$('deviceTimezone').value,connectionMode:'LOCAL_CONNECTOR'});await metaApi({action:'saveMetadata',deviceId:result.deviceId,model:$('deviceModel').value,serialNumber:$('deviceSerial').value,protocolMode:$('deviceProtocol').value,connectorPort:Number($('deviceConnectorPort').value||8088)});data=result;await reloadMeta();$('deviceName').value='';$('deviceLocation').value='';$('deviceSerial').value='';render();setStatus('ZKTeco устройство добавлено','ok')}catch(e){$('tcError').hidden=false;$('tcError').textContent=e.message;setStatus('Ошибка','error')}}
  ...
  async function revokeDeviceToken(deviceId){if(!confirm('Отозвать ключ устройства? Connector сразу потеряет доступ к отправке событий.'))return;try{setStatus('Отзываем ключ…','loading');data=await api({action:'revokeDeviceToken',deviceId});mergeMeta();hideDeviceToken();render();setStatus('Ключ отозван','ok')}catch(e){$('tcError').hidden=false;$('tcError').textContent=e.message;setStatus('Ошибка','error')}}
  ...
  function bind(){$('tcRefresh').onclick=load;$('deviceForm').onsubmit=saveDevice;$('bindingForm').onsubmit=link;$('copyDeviceToken').onclick=copyDeviceToken;$('copyDeviceConfig').onclick=copyDeviceConfig;$('closeDeviceToken').onclick=hideDeviceToken}
