...
function uid(prefix='hct'){return `${prefix}_${crypto.randomUUID()}`}
...
async function ensure(db){
  ...
  await db.batch([
    ...
    db.prepare(`CREATE INDEX IF NOT EXISTS idx_hr_role_comp_terms_role ON hr_role_compensation_terms(user_id,role_code,is_active,effective_from)`),
    db.prepare(`CREATE UNIQUE INDEX IF NOT EXISTS idx_hr_role_comp_terms_unique_start ON hr_role_compensation_terms(user_id,role_code,effective_from)`)
  ]);
}

function commonTerm(r){return{effectiveFrom:r.effective_from,effectiveTo:r.effective_to||'',officialGross:money(r.official_gross),additionalAmount:money(r.additional_amount),additionalPaymentMethod:r.additional_payment_method||'CASH',additionalTaxTreatment:r.additional_tax_treatment||'TAXABLE',additionalLegalBasis:r.additional_legal_basis||'',note:r.note||'',active:Boolean(r.is_active),createdAt:r.created_at,updatedAt:r.updated_at}}
function employeeTermDto(r){return{id:r.term_id,scopeType:'EMPLOYEE',employeeId:r.iiko_employee_id,...commonTerm(r)}}
function roleTermDto(r){return{id:r.term_id,scopeType:'ROLE',roleCode:r.role_code,...commonTerm(r)}}
...
async function snapshot(db,userId,asOf){
  ...
  const roles=baseRoles.map(r=>{
    const term=roleCurrent.get(String(r.code))||null;
    return{...r,employeeCount:employeeCountByRole.get(String(r.code))||0,term,calculation:calculationFor(term,asOf)};
  });

  const employees=employeeRows.map(e=>{
    const id=String(e.iiko_employee_id),roleCode=String(e.role_code||'');
    const individualTerm=employeeCurrent.get(id)||null;
    const inheritedRoleTerm=roleCode?roleCurrent.get(roleCode)||null:null;
    const term=individualTerm||inheritedRoleTerm;
    const sourceType=individualTerm?'EMPLOYEE':(inheritedRoleTerm?'ROLE':'');
    const full=[e.last_name,e.first_name,e.middle_name].filter(Boolean).join(' ')||e.display_name||e.employee_code||e.iiko_employee_id;
    return{id:e.iiko_employee_id,code:e.employee_code,name:full,roleCode,roleName:e.role_name||'',departmentCode:e.department_code||'',hireDate:e.hire_date||'',fireDate:e.fire_date||'',term,individualTerm,inheritedRoleTerm,sourceType,calculation:calculationFor(term,asOf)};
  });
...
function validatePaymentFields(body){
  const effectiveFrom=dateOnly(body.effectiveFrom),effectiveTo=dateOnly(body.effectiveTo),officialGross=money(body.officialGross),additionalAmount=money(body.additionalAmount),method=(clean(body.additionalPaymentMethod)||'CASH').toUpperCase(),treatment=(clean(body.additionalTaxTreatment)||'TAXABLE').toUpperCase(),basis=clean(body.additionalLegalBasis),note=clean(body.note);
  if(!effectiveFrom)return{error:'Укажите дату начала действия'};
  if(effectiveTo&&effectiveTo<effectiveFrom)return{error:'Дата окончания не может быть раньше даты начала'};
  if(!['CASH','BANK','OTHER'].includes(method))return{error:'Неизвестный способ выплаты'};
  if(!['TAXABLE','EXEMPT_WITH_BASIS'].includes(treatment))return{error:'Неизвестный налоговый режим дополнительной выплаты'};
  if(treatment==='EXEMPT_WITH_BASIS'&&!basis)return{error:'Для необлагаемой выплаты обязательно укажите законное основание'};
  return{effectiveFrom,effectiveTo,officialGross,additionalAmount,method,treatment,basis,note};
}

async function roleExists(db,userId,roleCode){
  const fromEmployees=await db.prepare(`SELECT role_code FROM hr_employees WHERE user_id=?1 AND role_code=?2 AND is_deleted=0 LIMIT 1`).bind(userId,roleCode).first().catch(()=>null);if(fromEmployees)return true;
  const fromRoles=await db.prepare(`SELECT role_code FROM hr_roles WHERE user_id=?1 AND role_code=?2 AND is_deleted=0 LIMIT 1`).bind(userId,roleCode).first().catch(()=>null);return Boolean(fromRoles);
}
...
export async function onRequestPost({request,env}){
  try{
    ...
    if(action==='saveTerm'){
      const scopeType=(clean(body.scopeType)||'EMPLOYEE').toUpperCase(),fields=validatePaymentFields(body);if(fields.error)return json({success:false,message:fields.error},400);
      ...
      if(scopeType==='ROLE'){
        ...
        await env.DB.prepare(`INSERT INTO hr_role_compensation_terms(user_id,term_id,role_code,effective_from,effective_to,official_gross,additional_amount,additional_payment_method,additional_tax_treatment,additional_legal_basis,note,is_active,created_at,updated_at)
