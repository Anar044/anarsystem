...
async function seedRolesFromEmployees(db,userId){
  ...
  for(const r of rows.results||[]){
    statements.push(db.prepare(`INSERT OR IGNORE INTO hr_roles(user_id,iiko_role_id,role_code,role_name,is_deleted,synced_at) VALUES(?1,'',?2,?3,0,?4)`).bind(userId,clean(r.role_code),clean(r.role_name)||clean(r.role_code),t));
  }
...
async function snapshot(db,userId){
  ...
  const [rolesResult,schedulesResult,countsResult]=await Promise.all([
  ...
  const schedules=(schedulesResult.results||[]).map(s=>({
    ...
    weekdays:String(s.weekdays||'').split(',').map(Number).filter(Boolean),workDays:Number(s.work_days||0),offDays:Number(s.off_days||0),anchorDate:s.anchor_date||'',
    shiftStart:s.shift_start,shiftEnd:s.shift_end,breakMinutes:Number(s.break_minutes||0),validFrom:s.valid_from,validTo:s.valid_to||'',
  ...
  return{roles,schedules,counts:{roles:activeRoles.length,employees:activeRoles.reduce((a,r)=>a+r.employeeCount,0),schedules:schedules.filter(x=>x.active).length,rolesWithoutDefault:activeRoles.filter(r=>!r.hasDefaultSchedule).length}};
}
...
export async function onRequestPost({request,env}){
  try{
    ...
    if(action==='saveSchedule'){
      ...
      const workDays=int(body.workDays,1,14,5),offDays=int(body.offDays,1,14,2),anchorDate=dateOnly(body.anchorDate),isDefault=Boolean(body.isDefault);
      ...
      return json({success:true,scheduleId,...await snapshot(env.DB,userId)});
    }
