...
function heartbeatOnline(value){if(!value)return false;const ms=Date.now()-new Date(value).getTime();return Number.isFinite(ms)&&ms>=0&&ms<90000}
...
async function snapshot(db,userId){
  const [devices,employees,bindings,events,tokens,heartbeats]=await Promise.all([
    ...
    db.prepare(`SELECT * FROM hr_device_heartbeats WHERE user_id=?1`).bind(userId).all()
  ]);
  const ds=devices.results||[],es=employees.results||[],bs=bindings.results||[],ev=events.results||[],ts=tokens.results||[],hs=heartbeats.results||[];
  const tokenMap=new Map(ts.map(x=>[String(x.device_id),x]));
  const heartbeatMap=new Map(hs.map(x=>[String(x.device_id),x]));
  return{
    devices:ds.map(x=>{
      const token=tokenMap.get(String(x.device_id));
      const hb=heartbeatMap.get(String(x.device_id));
      return{
        id:x.device_id,
        provider:x.provider,
        name:x.name,
        location:x.location,
        connectionMode:x.connection_mode,
        timezone:x.timezone,
        active:Boolean(x.is_active),
        lastSyncAt:x.last_sync_at||'',
        tokenConfigured:Boolean(token),
        tokenLastUsedAt:token?.last_used_at||'',
        tokenRotatedAt:token?.rotated_at||'',
        connectorOnline:heartbeatOnline(hb?.last_seen_at),
        connectorLastSeenAt:hb?.last_seen_at||'',
        connectorVersion:hb?.connector_version||'',
        serialNumber:hb?.serial_number||'',
        model:hb?.model||'',
        ipAddress:hb?.ip_address||'',
        port:Number(hb?.port)||0,
        protocol:hb?.protocol||'',
        queueSize:Number(hb?.queue_size)||0,
        terminalLastSeenAt:hb?.terminal_last_seen_at||''
      }
    }),
    ...
    counts:{devices:ds.filter(x=>x.is_active).length,employees:es.filter(x=>!x.is_deleted&&!x.fire_date).length,bindings:bs.length,events:ev.length,unmatchedEvents:ev.filter(x=>!x.iiko_employee_id).length,deviceTokens:ts.length,connectorsOnline:hs.filter(x=>heartbeatOnline(x.last_seen_at)).length}
  };
}
...
export async function onRequestGet({request,env}){
  try{const a=await auth(request,env);if(!a)return json({success:false,message:'Требуется авторизация'},401);return json({success:true,attendanceSource:'EXTERNAL_DEVICE',payrollEngine:'SMART_HORECA',ingestPath:'/api/hr/device-ingest',heartbeatPath:'/api/hr/device-heartbeat',...(await snapshot(env.DB,a.user.id))})}
...
export async function onRequestPost({request,env}){
  try{
    ...
    if(action==='saveDevice'){
      ...
      return json({success:true,deviceId:id,ingestPath:'/api/hr/device-ingest',heartbeatPath:'/api/hr/device-heartbeat',...await snapshot(env.DB,userId)});
    }
    if(action==='rotateDeviceToken'){
      ...
      return json({success:true,deviceId,deviceToken:token,ingestPath:'/api/hr/device-ingest',heartbeatPath:'/api/hr/device-heartbeat',...await snapshot(env.DB,userId)});
    }
    if(action==='revokeDeviceToken'){
      ...
      await env.DB.prepare(`DELETE FROM hr_device_heartbeats WHERE user_id=?1 AND device_id=?2`).bind(userId,deviceId).run();
      return json({success:true,deviceId,ingestPath:'/api/hr/device-ingest',heartbeatPath:'/api/hr/device-heartbeat',...await snapshot(env.DB,userId)});
    }
    if(action==='linkEmployee'){
      ...
      return json({success:true,ingestPath:'/api/hr/device-ingest',heartbeatPath:'/api/hr/device-heartbeat',...await snapshot(env.DB,userId)});
    }
    ...
    if(action==='importEvents'){
      ...
      return json({success:true,imported:accepted,skipped,ingestPath:'/api/hr/device-ingest',heartbeatPath:'/api/hr/device-heartbeat',...await snapshot(env.DB,userId)});
    }
