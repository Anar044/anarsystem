...
async function snapshot(db,userId,month){
  ...
  const adjustments=(adjustmentsR.results||[]).map(adjustmentDto),policies=policiesR.results||[];
  const mealMap=new Map((mealR.results||[]).map(x=>[String(x.iiko_employee_id).toLowerCase(),x]));
  const empTerms=employeeTermsR.results||[],roleTerms=roleTermsR.results||[],employees=[];
  for(const e of employeesR.results||[]){
    ...
    const id=String(e.iiko_employee_id),idKey=id.toLowerCase(),roleCode=String(e.role_code||''),name=[e.last_name,e.first_name,e.middle_name].filter(Boolean).join(' ')||e.display_name||e.employee_code||id;
    const list=adjustments.filter(a=>String(a.employeeId)===id),approved=list.filter(a=>a.status==='APPROVED');
    const advances=approved.filter(a=>a.type==='ADVANCE').reduce((s,a)=>s+a.amount,0);
    const deductions=approved.filter(a=>a.type==='DEDUCTION').reduce((s,a)=>s+a.amount,0);
    const taxableRewards=approved.filter(a=>a.type==='REWARD'&&a.taxTreatment!=='EXEMPT_WITH_BASIS').reduce((s,a)=>s+a.amount,0);
    const exemptRewards=approved.filter(a=>a.type==='REWARD'&&a.taxTreatment==='EXEMPT_WITH_BASIS').reduce((s,a)=>s+a.amount,0);
    ...
    const beforeDeductions=calc?money(calc.totalEmployeeReceives+exemptRewards):0;
    const finalPayable=money(Math.max(0,beforeDeductions-advances-deductions));
    const employerCost=calc?money(calc.totalEmployerCost+exemptRewards):0;
    const policy=activePolicy(policies,id,roleCode,asOf),limit=money(policy?.monthly_limit||0),meal=mealMap.get(idKey);
    const openingDebt=money(meal?.opening_debt||0),closingDebt=money(meal?.closing_debt||0),netIncrease=Math.max(0,Number(meal?.month_net_increase||0));
    ...
    employees.push({id,code:e.employee_code||'',name,roleCode,roleName:e.role_name||roleCode,
      adjustments:list,
      totals:{advances:money(advances),deductions:money(deductions),taxableRewards:money(taxableRewards),exemptRewards:money(exemptRewards),rewards:money(taxableRewards+exemptRewards),drafts:list.filter(a=>a.status==='DRAFT').length},
      meal:{policyId:policy?.policy_id||'',policySource:policy?.scope_type||'',limit,openingDebt,closingDebt,monthNetIncrease:money(netIncrease),restaurantCovered,overLimit,approvedDeduction:money(approvedMealDeduction),pendingDeduction:money(Math.max(0,overLimit-approvedMealDeduction)),source:meal?.source||'',syncedAt:meal?.synced_at||''},
  ...
  return{success:true,month,employees,adjustments,
...
function aggregateBalances(rows,employeeIds,debtAccountIds){
  ...
  for(const r of rows){const employeeId=rowEmployeeId(r),accountId=rowAccountId(r);if(!employeeIds.has(employeeId))continue;if(!debtAccountIds.has(accountId))continue;out.set(employeeId,(out.get(employeeId)||0)+rowAmount(r));matched++}
  return{out,matched};
}
async function syncIikoDebt(request,env,userId,month){
  ...
  const [openRows,closeRows]=await Promise.all([fetchCounteragentBalances(connection,openTs),fetchCounteragentBalances(connection,closeTs)]);
  const open=aggregateBalances(openRows,ids,accounts.ids),close=aggregateBalances(closeRows,ids,accounts.ids),t=now(),statements=[];
...
async function requireDeductionBasis(db,userId,id){
  const row=await db.prepare(`SELECT adjustment_type,legal_basis FROM hr_payroll_adjustments WHERE user_id=?1 AND adjustment_id=?2 LIMIT 1`).bind(userId,id).first();
  if(!row)throw new Error('Операция не найдена');
  if(row.adjustment_type==='DEDUCTION'&&!clean(row.legal_basis))throw new Error('Для подтверждения удержания укажите основание/письменное согласие или другой допустимый документ.');
}
...
export async function onRequestGet({request,env}){try{
  ...
  return json(await snapshot(env.DB,state.user.id,month));
...
export async function onRequestPost({request,env}){try{
  ...
  if(action==='saveAdjustment'){
    ...
    if(!['DRAFT','APPROVED'].includes(status))return json({success:false,message:'Некорректный статус'},400);
    ...
    const emp=await env.DB.prepare(`SELECT 1 FROM hr_employees WHERE user_id=?1 AND iiko_employee_id=?2 AND is_deleted=0 LIMIT 1`).bind(userId,employeeId).first();if(!emp)return json({success:false,message:'Сотрудник не найден'},404);
    ...
    return json(await snapshot(env.DB,userId,month));
  }
  if(action==='setAdjustmentStatus'){
    ...
    await env.DB.prepare(`UPDATE hr_payroll_adjustments SET status=?3,updated_at=?4 WHERE user_id=?1 AND adjustment_id=?2`).bind(userId,id,status,t).run();return json(await snapshot(env.DB,userId,month));
  }
  ...
  if(action==='proposeMealDeduction'){
    const employeeId=clean(b.employeeId),snap=await snapshot(env.DB,userId,month),e=snap.employees.find(x=>String(x.id)===employeeId);if(!e||e.meal.pendingDeduction<=0)return json({success:false,message:'Нет сверхлимитной суммы для удержания'},400);
    const id=uid('hpa');await env.DB.prepare(`INSERT INTO hr_payroll_adjustments(user_id,adjustment_id,iiko_employee_id,month,adjustment_type,amount,reason,source,tax_treatment,legal_basis,status,created_at,updated_at) VALUES(?1,?2,?3,?4,'DEDUCTION',?5,'Питание сверх месячного лимита','MEAL_OVER_LIMIT','TAXABLE','','DRAFT',?6,?6)`).bind(userId,id,employeeId,month,e.meal.pendingDeduction,t).run();return json(await snapshot(env.DB,userId,month));
  }
