...
export async function onRequestPost({request,env}){
  try{
    ...
    if(body?.heartbeat===true){
      await env.DB.batch([
        env.DB.prepare(`UPDATE hr_devices SET last_connector_at=?3 WHERE user_id=?1 AND device_id=?2`).bind(row.user_id,row.device_id,seenAt),
        env.DB.prepare(`UPDATE hr_device_tokens SET last_used_at=?3 WHERE user_id=?1 AND device_id=?2`).bind(row.user_id,row.device_id,seenAt)
      ]);
      return json({success:true,heartbeat:true,deviceId:row.device_id,deviceName:row.name,receivedAt:seenAt});
    }
    ...
    const importedAt=seenAt;let accepted=0,invalid=0,matched=0,unmatched=0;const statements=[];
    ...
    await env.DB.batch([
      env.DB.prepare(`UPDATE hr_devices SET last_sync_at=?3,last_connector_at=?3,updated_at=?3 WHERE user_id=?1 AND device_id=?2`).bind(row.user_id,row.device_id,importedAt),
