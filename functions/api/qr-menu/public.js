...
export async function onRequestGet({ request, env }) {
  try {
    ...
    const menu = await env.DB.prepare(`
      ...
      FROM qr_menus
      WHERE public_slug = ?1 AND is_published = 1
      LIMIT 1
    `).bind(publicId).first();
    ...
    const dishes = await env.DB.prepare(`
      SELECT id, category_id, iiko_id, name, description,
             composition, price, currency, photo_url,
             sort_order, is_available
      FROM qr_dishes
      WHERE menu_id = ?1 AND is_available = 1
      ORDER BY sort_order ASC, name ASC
    `).bind(menu.id).all();
    ...
    return json({
      ...
      menu: {
        ...
        dishes: (dishes.results || []).map(d => ({
          id: d.id,
          iikoId: d.iiko_id,
          cat: d.category_id,
          name: d.name,
          desc: d.description || d.composition || "",
